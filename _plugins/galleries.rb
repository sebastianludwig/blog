...
module Jekyll
	...
	class GalleryTag < Liquid::Block
		...
		def gallery_images(bock_content)
			...
			input_data.each do |item|
				next if File.basename(item[0]).include? "-thumb"
			    ...
			    if File.directory?(full_item_path) && item_ok   # check if this item points to a directory
			        itemnum = 1
                    Dir.glob(File.join(full_item_path, "*.{png,jpg,jpeg,gif}"), File::FNM_CASEFOLD).each do |file|
	...
	class ThumbGenerator < Generator
	 	...
	 	def files_to_resize(site)
	 		...
	 		Dir.glob(File.join(@gallery_dir, "**", "*.{png,jpg,jpeg,gif}"), File::FNM_CASEFOLD).each do |file|
	 			next if File.basename(file).include? "-thumb"
