...
- Fuzzy edges (half pixel) are still a thing (even with retina)
    -> create 2x PDFs and drop it in the 2x-slot (see 31:40)
...
- Images
    - Downsampling: `ImageIO` uses streaming IO and prevents the need to decode the complete source image (also see 219)
